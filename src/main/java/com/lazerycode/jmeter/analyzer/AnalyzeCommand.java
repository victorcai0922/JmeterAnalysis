...
public class AnalyzeCommand {
    ...
    if (testResults.size() == 0) {
      ...
      if(generateCSVs || generateCharts) {
        ...
        for (Map.Entry<String, AggregatedResponses> entry : testResults.entrySet()) {
           ...
           */
          writeDatabase(projectName, username, pwd, host, database, aggregatedResponses);
  ...
  /**
   * Generate Database result
   * @param username
   * @param pwd
   * @param host
   * @param database
   * @param aggregatedResponses
 * @throws UnsupportedEncodingException 
   */
  private void writeDatabase(String name,String username, String pwd, String host, String database,AggregatedResponses aggregatedResponses) throws UnsupportedEncodingException{
	  String projectName = urlEncode(name);
	  ResultRenderHelper.renderDatabase(projectName, username, pwd, host, database, aggregatedResponses);
  }
